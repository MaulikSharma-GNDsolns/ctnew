...
import { View, Text, TouchableOpacity, ScrollView, SafeAreaView, StatusBar, ActivityIndicator, Modal } from 'react-native';
...
export default function App() {
    ...
    const { connectToDevice, disconnect, reset, connectedDevice, isConnecting, lastPacket, statusMessage, mqttStatus } = useBLE();
                        ...
                        <View className="flex-row justify-between items-center mt-12 mb-8">
                            <View className="flex-row items-center">
                                ...
                                <View>
                                    ...
                                    <View className="flex-row items-center mt-1">
                                        ...
                                        <Text className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mr-4">
                                            {statusMessage}
                                        </Text>

                                        {/* MQTT Status */}
                                        <View className={`w-1.5 h-1.5 rounded-full mr-2 ${mqttStatus ? 'bg-cyan-400 shadow-sm shadow-cyan-200' : 'bg-slate-200'}`} />
                                        <Text className={`text-[10px] font-bold uppercase tracking-widest ${mqttStatus ? 'text-cyan-500' : 'text-slate-300'}`}>
                            ...
                            {!connectedDevice && (
                                <TouchableOpacity 
                                    onPress={() => {
                                        setShowScanner(true);
                                        mqttService.sendTestMessage(); // Modular test message
                                    }}
